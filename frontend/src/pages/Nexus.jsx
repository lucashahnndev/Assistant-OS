...
import { ThoughtInspectorCard, HistoryInspectorCard, WorkerInspectorCard } from '../components/SystemInspectorCards';
...
const useMediaStackManager = (preferredStageSignatures = [], sessionId = null) => {
    const [mediaList, setMediaList] = useState(() => {
        try {
            const saved = JSON.parse(localStorage.getItem('atlas_media_workstation') || '[]');
            const now = Date.now();
            return saved.filter(m => {
                if (['TERMINAL', 'APPROVAL', 'PLAYBACK', 'CODE'].includes(String(m.type).toUpperCase())) return false;
                if (m.isPinned) return true;
                if (m.status === 'expired') return false;
                const totalDuration = m.payload?.duration || m.durationMs || HERO_CONSTANTS.MEDIA_EXPIRE_DURATION_MS;
                const rem = (m.expiresAt || (m.createdAt + totalDuration)) - now;
                if (rem > 0) {
                    m.remainingTime = rem;
                    m.expiresAt = now + rem;
                    return true;
    ...
    useEffect(() => {
        // Restart timers for loaded non-pinned media
        mediaList.forEach(m => {
            if (!m.isPinned && m.remainingTime > 0 && !timersRef.current.has(`${m.id}_expire`) && m.status !== 'focused') {
                const t = setTimeout(() => removeMedia(m.id), m.remainingTime);
                timersRef.current.set(`${m.id}_expire`, t);
            }
        });
    }, []);
    ...
    const addMedia = (itemPayload, type = 'IMAGE', isPersistent = false) => {
        ...
        if (incomingSignature) {
            ...
            if (existingBySignature?.id) {
                const existingId = existingBySignature.id;
                clearMediaTimer(`${existingId}_focus`);
                clearMediaTimer(`${existingId}_expire`);
                setMediaList(prev => prev.map(m => {
                    if (m.id === existingId) {
                        return {
                            ...
                            createdAt: now,
                            status: 'focused',
                            payload: { ...(m.payload || {}), ...(itemPayload || {}) },
                            remainingTime: isPersistent ? null : HERO_CONSTANTS.MEDIA_EXPIRE_DURATION_MS,
                            expiresAt: isPersistent ? null : now + HERO_CONSTANTS.MEDIA_EXPIRE_DURATION_MS,
                            isPinned: isPersistent ? true : m.isPinned,
                        };
                    }
                    return m.status === 'focused' ? { ...m, status: 'docked' } : m;
                }));
                setFocusedMediaId(existingId);
                ...
                if (!isPersistent) {
                    const t = setTimeout(() => dockMedia(existingId), HERO_CONSTANTS.MEDIA_FOCUS_DURATION_MS);
                    timersRef.current.set(`${existingId}_focus`, t);
                }
                return existingId;
            }
        }
        ...
        const newItem = {
            id, type, createdAt: now, status: 'focused', payload: itemPayload,
            expiresAt: isPersistent ? null : now + HERO_CONSTANTS.MEDIA_EXPIRE_DURATION_MS,
            remainingTime: isPersistent ? null : HERO_CONSTANTS.MEDIA_EXPIRE_DURATION_MS,
    ...
    const pauseMediaExpiry = (id) => clearMediaTimer(`${id}_expire`);
    ...
    const stageItems = useMemo(() => {
        ...
        const withPriorityMeta = active
            .map((m) => {
                ...
                score += Math.floor(Number(m?.createdAt || 0) / 1000);
    ...
    if (!isExpanded) {
        return (
            <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                <button onClick={() => setIsExpanded(true)} className="glass" style={{
                    padding: '6px 16px', borderRadius: '16px', fontSize: '10px', fontWeight: '800', letterSpacing: '0.05em',
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ width: '100%', position: 'relative' }}>
                <button onClick={() => setIsExpanded(false)} style={{
                    ...
                    borderRadius: '4px', color: 'var(--text-muted)', cursor: 'pointer', padding: '4px',
        ...
        >
            {popups.map(item => {
                ...
                const deezerExternalUrl = item.type === 'DEEZER'
                    ? String(deezerId ? `https://www.deezer.com/track/${deezerId}` : (item?.payload?.url || '')).trim()
                    : '';
                    ...
                    >
                        ...
                        <div style={{ width: '44px', height: '44px', borderRadius: '6px', overflow: 'hidden', flexShrink: 0, background: 'var(--bg-color)', position: 'relative', border: '1px solid rgba(255,255,255,0.12)' }}>
                            {item.type === 'IMAGE' ? (
                                ...
                                <div className="flex-center" style={{ height: '100%', background: 'rgba(148, 163, 184, 0.12)' }}><Terminal size={20} color="#cbd5e1" /></div>
                            ) : item.type === 'WEATHER' ? (
                            ...
                            ) : item.type === 'THOUGHT_INSPECTOR' ? (
                                <div className="flex-center" style={{ height: '100%', background: 'rgba(168, 85, 247, 0.12)' }}><Activity size={20} color="#a855f7" /></div>
                            ) : item.type === 'HISTORY_INSPECTOR' ? (
                                <div className="flex-center" style={{ height: '100%', background: 'rgba(59, 130, 246, 0.12)' }}><MessageSquare size={20} color="#3b82f6" /></div>
                            ) : item.type === 'WORKER_INSPECTOR' ? (
                                <div className="flex-center" style={{ height: '100%', background: 'rgba(16, 185, 129, 0.12)' }}><Layers size={20} color="#10b981" /></div>
                            ) : item.type === 'APPROVAL' ? (
                            ...
                            {(item.type === 'YOUTUBE' || item.type === 'DEEZER') && (
                                <div style={{
                                    width: '14px', height: '14px', borderRadius: '4px', position: 'absolute', right: '2px', bottom: '2px',
                                    border: '1px solid rgba(255,255,255,0.38)', background: platformBadgeBg, color: '#fff', fontSize: '8px',
                                    fontWeight: 800, lineHeight: '12px', textAlign: 'center', overflow: 'hidden'
                                }}>
                                    {platformBadgeLabel}
                                    {platformFaviconUrl && (
                                        <img
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', marginBottom: '12px' }}>
                ...
                <div style={{
                    padding: '4px 10px', borderRadius: '999px', background: statusPillBg, border: statusPillBorder,
                    color: statusPillColor, fontSize: '10px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em',
                }}>
...
const StageMediaLayer = ({ mediaState, onDockMedia, onResolveApproval, onOpenTerminalFullscreen, sessionId, isMobile = false }) => {
    ...
    const resolvedDeezerUrl = isDeezer
        ? String(item?.payload?.url || (item?.payload?.trackId ? `https://www.deezer.com/track/${item.payload.trackId}` : '')).trim()
        : '';
    const genericExternalUrl = String(item?.payload?.url || '').trim();
    const shellExternalUrl = isYouTube ? resolvedYouTubeUrl : isDeezer ? resolvedDeezerUrl : genericExternalUrl;
    const shellTitle = (
        isYouTube ? 'YouTube' :
            isDeezer ? 'Deezer' :
    ...
    const frameMaxHeight = isMobile
        ? (isWeather ? 'min(66vh, 560px)' : isAssistCard || isInspector ? 'min(72vh, 700px)' : isYouTube ? 'min(58vh, 460px)' : isDeezer ? 'min(66vh, 540px)' : isTerminal ? 'min(52vh, calc(100vh - 260px))' : isCode ? 'min(52vh, 500px)' : isImage ? 'min(42vh, 340px)' : 'min(30vh, 240px)')
        : (isWeather
            ? (isSingleStage ? 'min(72vh, 600px)' : 'min(64vh, 540px)')
            : isAssistCard || isInspector
                ? (isSingleStage ? 'min(76vh, 760px)' : 'min(68vh, 660px)')
                : isApproval
                    ? (isSingleStage ? 'min(48vh, 400px)' : 'min(44vh, 360px)')
                    : isYouTube
                        ? (isSingleStage ? 'min(56vh, 520px)' : 'min(48vh, 420px)')
                        : isDeezer
                            ? (isSingleStage ? 'min(62vh, 520px)' : 'min(56vh, 460px)')
                            : isTerminal
                                ? 'min(70vh, 620px)'
                                : isCode
                                    ? 'min(64vh, 560px)'
    ...
    return (
        <div className="glass" style={{
            ...
            animation: 'scaleUp 0.25s cubic-bezier(0.19, 1, 0.22, 1)', pointerEvents: 'auto',
            ...
            {/* Header Toolbar */}
            {!isApproval && (
                <div style={{ height: '34px', padding: '0 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255, 255, 255, 0.1)', background: 'rgba(255,255,255,0.03)' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                        {shellFavicon ? (
                            <img src={shellFavicon} alt="" style={{ width: '12px', height: '12px', borderRadius: '3px', flexShrink: 0 }} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                        ) : (
                            <Globe size={11} color="var(--accent-color)" />
                        )}
                        <span style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '0.08em', color: 'rgba(255, 255, 255, 0.8)', textTransform: 'uppercase' }}>
                            {shellTitle}
                        </span>
                    </div>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                        {shellExternalUrl && (
                            <>
                                <button onClick={handleCopyExternal} className="btn-ghost" style={{ padding: '4px', borderRadius: '6px' }} title="Copy External Link"><Copy size={11} strokeWidth={2.4} /></button>
                                <a href={shellExternalUrl} target="_blank" rel="noreferrer" className="btn-ghost" style={{ padding: '4px', display: 'flex', color: '#e2e8f0', borderRadius: '6px' }} title="Open Externally"><ExternalLink size={11} strokeWidth={2.4} /></a>
                            </>
                        )}
                        <button onClick={() => onDockMedia(item.id)} className="btn-ghost" style={{ padding: '4px', borderRadius: '6px' }} title="Dock Card"><Minimize2 size={11} /></button>
                    </div>
                </div>
            )}
            ...
            <div style={{ flex: 1, minHeight: 0, overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column' }}>
                ...
                {isTerminal && <TerminalStreamCard payload={item.payload} onOpenFullscreen={onOpenTerminalFullscreen} />}
                {isPlayback && <PlaybackCard runId={item.payload?.runId || item.payload?.run_id} sessionId={item.payload?.sessionId || sessionId} />}
                {isFile && (
                    <div className="flex-center" style={{ width: '100%', height: '100%', padding: '24px', background: 'rgba(2, 6, 18, 0.96)' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', textAlign: 'center' }}>
                            <Terminal size={34} color="#cbd5e1" />
                            <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>
                                {String(item.payload?.title || item.payload?.name || 'Attachment')}
                            </div>
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)', maxWidth: '420px', wordBreak: 'break-word' }}>
                                {String(item.payload?.path || item.payload?.file_path || item.payload?.url || '')}
                            </div>
                            {getFileUrl(item.payload, sessionId) && (
                                <a
                                    href={getFileUrl(item.payload, sessionId)}
                                    target="_blank"
                                    rel="noreferrer"
                                    className="btn-ghost"
                                    style={{ padding: '8px 12px', borderRadius: '8px', display: 'inline-flex', alignItems: 'center', gap: '8px', textDecoration: 'none' }}
                                >
                ...
                {isApproval && <DashboardApprovalCard item={item} sessionId={sessionId} onResolved={onResolveApproval} />}
                {isAssistCard && <StageAssistCard type={item.type} payload={item.payload} sessionId={sessionId} isMobile={isMobile} />}
                {item.type === 'THOUGHT_INSPECTOR' && <ThoughtInspectorCard data={item.payload} isStage={true} />}
                {item.type === 'HISTORY_INSPECTOR' && <HistoryInspectorCard data={item.payload} isStage={true} />}
                {item.type === 'WORKER_INSPECTOR' && <WorkerInspectorCard data={item.payload} isStage={true} />}
                {isLink && <LinkPreviewCard messageContent={item.payload?.fullContent || item.payload?.url || ''} isStage={true} />}
            </div>
        </div>
    );
};
        ...
        <div style={{ width: '100%', height: 'auto', display: 'flex', flexDirection: 'column', padding: '12px', background: 'linear-gradient(180deg, rgba(9, 14, 25, 0.95), rgba(6, 10, 18, 0.95))' }}>
            ...
            <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                <div style={{ width: '100%', maxWidth: '240px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {showHeader && (
                        <div style={{
                            width: '100%', height: '28px', padding: '0 8px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)',
                            background: 'linear-gradient(180deg, rgba(2, 6, 18, 0.95), rgba(2, 6, 18, 0.74))', display: 'flex',
                            alignItems: 'center', justifyContent: 'space-between'
                        }}>
                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', minWidth: 0 }}>
                                <img src="/api/favicon?url=https%3A%2F%2Fdeezer.com" alt="" style={{ width: '12px', height: '12px', borderRadius: '3px', flexShrink: 0 }} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                                <span style={{ fontSize: '11px', fontWeight: 800, letterSpacing: '0.03em', color: '#e5e7eb', textTransform: 'uppercase' }}>Deezer</span>
                            </div>
                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                                <button type="button" className="btn-ghost" onClick={handleCopyExternal} disabled={!externalUrl} title="Copy Deezer link" style={{ padding: '4px', lineHeight: 0, color: '#e2e8f0', borderRadius: '6px', opacity: externalUrl ? 1 : 0.45 }}><Copy size={12} strokeWidth={2.4} /></button>
                                <button type="button" className="btn-ghost" onClick={handleOpenExternal} disabled={!externalUrl} title="Open on Deezer" style={{ padding: '4px', lineHeight: 0, color: '#e2e8f0', borderRadius: '6px', opacity: externalUrl ? 1 : 0.45 }}><ExternalLink size={12} strokeWidth={2.4} /></button>
                            </div>
                        </div>
                    )}
                    <div style={{ width: '100%', aspectRatio: '1 / 1', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--card-border)', background: 'rgba(255,255,255,0.04)', position: 'relative' }}>
                        ...
                        <button className="btn-ghost" onClick={togglePlay} disabled={!track?.preview || loadingTrack} style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', width: '56px', height: '56px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(255,255,255,0.32)', background: 'rgba(2, 6, 18, 0.58)', backdropFilter: 'blur(4px)' }} title={isPlaying ? 'Pause' : 'Play'}>
                            {isPlaying ? <Pause size={20} /> : <Play size={20} fill="currentColor" />}
                        </button>

                        <div style={{ position: 'absolute', left: '8px', right: '8px', bottom: '8px', padding: '6px 8px', borderRadius: '8px', background: 'rgba(2, 6, 18, 0.48)', border: '1px solid rgba(255,255,255,0.16)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)' }}>
                            <div style={{ fontSize: '12px', fontWeight: 800, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{loadingTrack ? 'Loading track...' : (track?.title || 'Deezer Track')}</div>
                            <div style={{ fontSize: '10px', color: 'rgba(226,232,240,0.95)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: '1px' }}>{track?.artist || 'Unknown artist'}</div>
                        </div>
                    </div>
                    {isBuffering && <div style={{ fontSize: '9px', color: 'var(--accent-color)', marginTop: '-2px' }}>Buffering...</div>}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', minWidth: 0 }}>
                            <img src={platformIconUrl} alt="" style={{ width: '12px', height: '12px', borderRadius: '3px', flexShrink: 0 }} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                            <span style={{ fontSize: '9px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{platformLabel}</span>
                        </div>
                        <span style={{ fontSize: '9px', color: 'var(--text-muted)', fontFamily: '"JetBrains Mono","Fira Code",monospace' }}>{fmt(currentTime)} / {fmt(total)}</span>
                    </div>
                </div>
            </div>
        </div>
...
const TerminalStreamCard = ({ payload, onOpenFullscreen }) => {
    ...
    const transcript = String(payload?.transcript || payload?.output_full || payload?.output_tail || `$ ${String(payload?.command || 'shell command')}\n(waiting for output...)`);
    ...
    return (
        <div style={{ width: '100%', height: '100%', maxHeight: '100%', minHeight: 0, display: 'flex', flexDirection: 'column', background: 'rgba(2, 6, 18, 0.96)', overflow: 'hidden' }}>
            <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--card-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                <div style={{ minWidth: 0 }}>
                    <div style={{ fontFamily: '"JetBrains Mono","Fira Code",monospace', fontSize: '11px', color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{String(payload?.command || payload?.terminal_id || 'terminal')}</div>
                    <div style={{ fontFamily: '"JetBrains Mono","Fira Code",monospace', fontSize: '10px', color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{String(payload?.cwd || '')}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                    <span style={{ fontSize: '9px', color: 'var(--text-muted)' }}>{Number(payload?.line_count || 0)} lines</span>
                    <button className="btn-ghost" style={{ padding: '4px', borderRadius: '6px' }} title="Abrir terminal em tela cheia" onClick={() => onOpenFullscreen?.()}><Maximize2 size={12} /></button>
            ...
            <div ref={scrollRef} className="custom-scrollbar" style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px' }}>
                <pre style={{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontSize: '12px', lineHeight: 1.5, color: '#d9e1ff', fontFamily: '"JetBrains Mono","Fira Code",monospace' }}>{transcript}</pre>
            </div>
        </div>
    );
};

// Extract weg blocks from conversation transcript
const extractWegScript = (history) => {
    const assistantMessages = history.filter(m => m.role === 'assistant' || m.role === 'atlas');
    ...
    // 2. Se o bloco estiver aberto mas não fechado (durante digitação em tempo real / streaming)
    const openWegMatch = content.match(/```weg\s*([\s\S]*)$/i);
    if (openWegMatch) return openWegMatch[1].trim();

    // 3. Se não há a tag ```weg, filtra e retorna APENAS as linhas que contêm instruções do compilador,
    // descartando qualquer texto humano livre ("Imagine the internet...") que poluiria o motor!
    if (content.includes('@Node') || content.includes('@World')) {
        const lines = content.split('\n');
        const compiledLines = lines.filter(line => {
            const trimmed = line.trim();
            return trimmed.startsWith('@Node') || trimmed.startsWith('@World') || trimmed.startsWith('#');
        });
...
const Nexus = () => {
    ...
    const msm = useMediaStackManager(preferredStageSignatures, state.textState?.sessionId);

    // Track state elements in refs to avoid double captures
    ...
    useEffect(() => {
        msmAddRef.current = msm.addMedia;
        msmFocusRef.current = msm.focusMedia;
        msmRemoveRef.current = msm.removeMedia;
        msmPatchPayloadRef.current = msm.patchMediaPayload;
    ...
    const assistCardSlotsByWorkRef = useRef(new Map());
    ...
    useEffect(() => {
        ...
        try {
            ...
            const arr = Array.isArray(parsed?.signatures)
                ? parsed.signatures.map((v) => String(v || '').trim()).filter(Boolean)
                : [];
    ...
    // Extract weg scripts from chat history automatically
    useEffect(() => {
        const script = extractWegScript(state.textState.history);
        setWegScript(script);
    }, [state.textState.history]);

    const claimAssistCardSlot = (workId, type) => {
        if (!workId) return true;
        const key = String(workId);
        const gate = assistCardSlotsByWorkRef.current;
    ...
    const handleApprovalResolved = (mediaId, workId) => {
        if (mediaId) msmRemoveRef.current(mediaId);
        if (workId) approvalCardByWorkRef.current.delete(String(workId));
    };

    const handlePopupClose = (mediaId) => {
        if (!mediaId) return;
    ...
    const stopTerminalTracking = useCallback((workId) => {
        const key = String(workId || '');
        if (!key) return;
        terminalWorksRef.current.delete(key);
    ...
    const startTerminalTracking = useCallback((workId) => {
        ...
        terminalWorksRef.current.add(key);
        ...
        const poll = async () => {
            try {
                ...
                // Keep silent while tracker is active.
            }
        };
    ...
    const syncPlaybackCard = (rawRunId, payloadPatch = {}) => {
        ...
        const fallbackMatch = currentList.find((m) => {
            if (m.type !== 'PLAYBACK') return false;
            const payloadRunId = String(m.payload?.runId || m.payload?.run_id || '').trim();
            return payloadRunId === runId;
        });
        if (fallbackMatch?.id) {
            msmPatchPayloadRef.current?.(fallbackMatch.id, normalizedPatch);
            map.set(runId, { mediaId: fallbackMatch.id, autoFocused: shouldAutoFocus });
            if (shouldAutoFocus) msmFocusRef.current?.(fallbackMatch.id);
            return;
        }
    ...
    const routeStructuredArtifacts = (msg, activeSessionId) => {
        const payload = (msg && typeof msg === 'object') ? msg : {};
        const workId = payload.work_id || payload?.context?.work_id || null;
        const sessionForAssets = String(activeSessionId || state.textState.sessionId || '').trim();

        const playback = (payload.playback && typeof payload.playback === 'object') ? payload.playback : null;
        if (playback?.run_id) {
            syncPlaybackCard(playback.run_id, {
                sessionId: String(playback.session_id || sessionForAssets || ''),
                status: String(playback.status || 'running'),
                work_id: workId,
        ...
        attachments.forEach((attachment) => {
            ...
            if (attachmentType === 'image') {
                msmRef.current.addMedia(normalized, 'IMAGE');
                return;
            }

            const resolvedUrl = getFileUrl(attachment, sessionForAssets);
            if (resolvedUrl) {
                msmRef.current.addMedia({ ...normalized, url: resolvedUrl }, 'FILE');
            }
        });
    };
    ...
    useEffect(() => {
        ...
        terminalWorksRef.current.clear();
        approvalCardByWorkRef.current.clear();
        assistCardSlotsByWorkRef.current.clear();
        playbackCardByRunRef.current.clear();
        deezerHydrationAttemptedRef.current.clear();
    }, [state.textState.sessionId]);

    // Play TTS chunks from websocket
    const playNextChunk = useCallback(async () => {
        ...
        try {
            if (!audioContextRef.current) {
                ...
                ttsAnalyserRef.current.fftSize = 32;
    ...
    return (
        <div className="dashboard-root" style={{
            ...
            <main style={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ position: 'absolute', bottom: '20px', right: '20px', zIndex: 11000, display: 'flex', flexDirection: 'column-reverse', alignItems: 'flex-end', gap: '10px' }}>
                    <button
                        ...
                        style={{
                            background: 'rgba(10, 12, 22, 0.72)', backdropFilter: 'blur(10px)',
                    ...
                    {settingsOpen && (
                        <div style={{
                            ...
                            padding: '16px', width: '260px', display: 'flex', flexDirection: 'column', gap: '14px',
                ...
                {(wegScript || sceneStreamActive) && (
                    <div style={{
                        ...
                        <div className="glass" style={{
                            padding: '10px 14px', borderRadius: HERO_CONSTANTS.RADIUS.CARD,
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    borderRadius: '6px', color: '#ef4444', padding: '4px 8px', cursor: 'pointer',
                                    fontSize: '10px', display: 'inline-flex', alignItems: 'center', gap: '4px',
                                    transition: 'all 0.2s'
                                }}
                                title="Limpar cena e voltar à orbe"
                            >
                                <Trash2 size={10} />
                                Limpar
                            </button>
                ...
                <div style={{ flex: 1, width: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
                    <div style={{
                        ...
                        paddingRight: msm.mediaState.popups.length > 0 && !isMobile ? '260px' : '0',
                        transition: 'padding-right 0.3s cubic-bezier(0.19, 1, 0.22, 1)'
                    }}>
                        <StageMediaLayer
                    ...
                    <div style={{
                        ...
                        <div style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '0 10px' : '0 40px', gap: isMobile ? '8px' : '16px', pointerEvents: 'auto' }}>
                            <div className="flex-center" style={{ gap: isMobile ? '6px' : '8px' }}>
                                <button
                                    ...
                                    style={{
                                        ...
                                        background: state.voiceMode === 'keyword' ? 'var(--accent-color)' : 'rgba(255, 255, 255, 0.08)',
                                        border: '1px solid rgba(255, 255, 255, 0.16)',
                                        color: state.voiceMode === 'keyword' ? '#000' : '#ffffff',
            ...
            {fullscreenTerminalPayload && createPortal(
                <div
                    style={{
                        position: 'fixed', inset: 0, zIndex: DASHBOARD_Z.FULLSCREEN_TERMINAL,
                ...
                >
                    <div
                        style={{
                            width: 'min(1200px, 100%)', height: 'min(86vh, 860px)',
                            border: '1px solid var(--card-border)', borderRadius: '12px',
                            overflow: 'hidden', background: 'rgba(2,6,18,0.98)',
                    ...
                    >
                        ...
                        <div className="custom-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '14px', background: 'rgba(4,7,20,0.8)' }}>
        ...
        if (this.state.hasError) {
            return (
                <div style={{
                    ...
                    height: '100vh', width: '100vw', background: 'radial-gradient(circle, #0e111a 0%, #050608 100%)',
                    color: '#e2e8f0', fontFamily: 'system-ui, sans-serif', padding: '24px', boxSizing: 'border-box'
                }}>
                    <div className="glass" style={{
                        maxWidth: '800px', width: '100%', padding: '32px', borderRadius: '16px',
                        border: '1px solid rgba(239, 68, 68, 0.3)', background: 'rgba(15, 23, 42, 0.65)',
                        boxShadow: '0 20px 40px rgba(0,0,0,0.5)', backdropFilter: 'blur(20px)',
                        display: 'flex', flexDirection: 'column', gap: '16px'
                    }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderBottom: '1px solid rgba(239, 68, 68, 0.2)', paddingBottom: '16px' }}>
                            <div style={{
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            ...
                            <div style={{
                                ...
                                borderRadius: '8px', padding: '16px', overflow: 'auto', maxHeight: '300px',
                                fontFamily: 'monospace', fontSize: '11px', lineHeight: '1.5', color: '#cbd5e1'
                            }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '8px' }}>
                            <button
                                ...
                                style={{
                                    background: 'var(--accent-color, #3b82f6)', border: 'none', borderRadius: '8px',
                                    color: '#fff', padding: '10px 20px', fontWeight: 700, cursor: 'pointer',
                                    fontSize: '12px', transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(59, 130, 246, 0.2)'
                                }}
                            >
                                Recarregar Página
                            </button>
