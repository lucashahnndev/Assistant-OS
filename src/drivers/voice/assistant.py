        ...
        if self.voice_recognition_engineering == 'vosk':
            ...
            try:
                ...
                with sr.Microphone(self.speech_recognition_engine_.microphone) as mic:
                    ...
                    self.rec.adjust_for_ambient_noise(mic, duration=0.5)
                    logger.debug('Ouvindo...')
                    audio = self.rec.listen(mic, timeout=10, phrase_time_limit=5)
                    logger.debug('Processando...')
                    text = self.rec.recognize_google(audio, language='pt-BR')
                    logger.info(f"Google Recognized: {text}")
                    return text
            except sr.WaitTimeoutError:
                logger.debug('Ouvindo: Tempo limite excedido (ninguém falou).')
                return None
            except sr.UnknownValueError:
                logger.debug('Não entendi o que você disse.')
                return None
