...
logger = logging.getLogger("WebSearchCapability")
...
class WebSearchCapability(CapabilityBase):
    ...
    def _router_config(self) -> Dict[str, Any]:
        ...
        out = dict(router_cfg)
        inherit_provider_capabilities = bool(
            out.get("inherit_provider_capabilities", cfg.get("inherit_provider_capabilities", True))
        )
        out["inherit_provider_capabilities"] = inherit_provider_capabilities

        if inherit_provider_capabilities:
            inferred = self._inferred_router_provider_configs()
            explicit = out.get("providers") if isinstance(out.get("providers"), dict) else {}
            merged: Dict[str, Any] = {}
            for provider_id in ("searxng", "brave", "ddg", "openalex", "commoncrawl"):
                base = inferred.get(provider_id) if isinstance(inferred.get(provider_id), dict) else {}
                override = explicit.get(provider_id) if isinstance(explicit.get(provider_id), dict) else {}
                if base or override:
                    merged[provider_id] = {**base, **override}
            if merged:
    ...
    def _inferred_router_provider_configs(self) -> Dict[str, Dict[str, Any]]:
        kernel = self.kernel
        ...
        def _cap_cfg(name: str) -> Dict[str, Any]:
            try:
                raw = cfg_manager.get_capability_config(name)
        ...
        searx_cfg = _cap_cfg("searxng_search")
        if searx_cfg:
            provider = searx_cfg.get("provider") if isinstance(searx_cfg.get("provider"), dict) else {}
            defaults = searx_cfg.get("defaults") if isinstance(searx_cfg.get("defaults"), dict) else {}
            inferred["searxng"] = {
                "enabled": bool(provider.get("enabled", True)),
                "endpoints": provider.get("endpoints"),
                "engines": provider.get("engines"),
                "timeout_ms": provider.get("timeout_ms") or defaults.get("timeout_ms"),
                "retries": provider.get("retries") or defaults.get("retries"),
            }

        brave_cfg = _cap_cfg("brave_search")
        if brave_cfg:
            provider = brave_cfg.get("provider") if isinstance(brave_cfg.get("provider"), dict) else {}
            defaults = brave_cfg.get("defaults") if isinstance(brave_cfg.get("defaults"), dict) else {}
            inferred["brave"] = {
        ...
        ddg_cfg = _cap_cfg("ddg_search")
        if ddg_cfg:
            provider = ddg_cfg.get("provider") if isinstance(ddg_cfg.get("provider"), dict) else {}
            inferred["ddg"] = {
                "enabled": bool(provider.get("enabled", False)),
            }

        openalex_cfg = _cap_cfg("openalex_search")
        if openalex_cfg:
            provider = openalex_cfg.get("provider") if isinstance(openalex_cfg.get("provider"), dict) else {}
            defaults = openalex_cfg.get("defaults") if isinstance(openalex_cfg.get("defaults"), dict) else {}
            inferred["openalex"] = {
                "enabled": bool(provider.get("enabled", True)),
                "api_base": provider.get("api_base"),
                "timeout_ms": provider.get("timeout_ms") or defaults.get("timeout_ms"),
        ...
        commoncrawl_cfg = _cap_cfg("commoncrawl_search")
        if commoncrawl_cfg:
            provider = commoncrawl_cfg.get("provider") if isinstance(commoncrawl_cfg.get("provider"), dict) else {}
            defaults = commoncrawl_cfg.get("defaults") if isinstance(commoncrawl_cfg.get("defaults"), dict) else {}
            inferred["commoncrawl"] = {
                "enabled": bool(provider.get("enabled", True)),
                "collinfo_url": provider.get("collinfo_url"),
                "max_indexes": provider.get("max_indexes") or defaults.get("max_indexes"),
                "timeout_ms": provider.get("timeout_ms") or defaults.get("timeout_ms"),
                "retries": provider.get("retries") or defaults.get("retries"),
            }
