...
- O capability recebe `exec_context` via `CapabilityRegistry.dispatch`.
- Campos relevantes hoje:
  - `session`: `_CapabilitySessionView` (somente leitura; sem `add_message`).
  - `callbacks`: somente `send_status` (demais callbacks nao sao expostos para capability).
...
- Implicacao: toda telemetria operacional do browser para supervisor/chat deve ir por:
  ...
  - `touch_work_context(...)`
  - retorno estruturado da capability (quando aplicavel)
...
- Compatibilidade com contrato atual de capability/kernel.
...
- Entregue:
  ...
  - `step` da capability implementado para continuidade com runtime ativo (lacuna critica fechada).
...
- Parcial:
  ...
  - Snapshot consolidado de abas foi adicionado ao retorno da capability, mas falta validacao E2E em ambiente real.
- Pendente:
...
### EPIC A - Hardening base da capability (pre-requisito)
- BC-A1: Corrigir schema `vision` em `ToonResponse`.
  - Arquivo: `src/capabilities/browser_control/schemas.py`
- BC-A2: Corrigir `playback` final para nao forcar `completed` em erro.
  - Arquivo: `src/capabilities/browser_control/browser_control_capability.py`
- BC-A3: Ajustar `contract.json` para refletir output real (`result` objeto).
  - Arquivo: `src/capabilities/browser_control/contract.json`
...
- BC-B1: Criar modulo de estado global `BrowserSessionRegistry`.
  - Arquivo novo: `src/capabilities/browser_control/session_registry.py`
- BC-B2: Definir schema de persistencia (`data/browser_registry.json`).
  - Arquivo novo: `src/capabilities/browser_control/registry_schema.py`
- BC-B3: Implementar lock/thread-safety e IO atomico.
  - Arquivo: `src/capabilities/browser_control/session_registry.py`
...
- BC-C2: Executar `reset_active_indexes_on_boot` no startup.
  - Arquivo: `src/main.py`
- BC-C3: Expor registry para orchestrator/capabilities via kernel.
  - Arquivo: `src/main.py`

### EPIC D - Policy automatica de roteamento
- BC-D1: Criar `BrowserSessionPolicy`.
  - Arquivo novo: `src/capabilities/browser_control/session_policy.py`
- BC-D2: Adicionar `intent_class` opcional no contrato da capability.
  - Arquivo: `src/capabilities/browser_control/contract.json`
...
- BC-D4: Porta CDP dinamica por instancia (sem hardcode 9222).
  - Arquivo: `src/capabilities/browser_control/runtime.py`
...
- BC-E2: Planner executar sempre no `tab_id` alvo resolvido.
  - Arquivo: `src/capabilities/browser_control/planner.py`
...
- BC-F1: Implementar deteccao de dominio elegivel para app mode.
  - Arquivo: `src/capabilities/browser_control/session_policy.py`
...
- BC-H1: Acoes internas para inventario e manutencao:
  ...
  - Arquivos: `src/capabilities/browser_control/contract.json`, `src/capabilities/browser_control/browser_control_capability.py`
...
### EPIC I - Contrato canonico de vision (sem quebrar outras capabilities)
- BC-I1: Definir resposta canonica para observacao visual consumivel por browser.
  - Arquivo novo: `src/capabilities/browser_control/vision_contract.py`
...
- `capabilities.browser_control.registry_enabled` (default: true)
- `capabilities.browser_control.policy_enabled` (default: false na fase inicial)
- `capabilities.browser_control.media_singleton_enforced` (default: false)
- `capabilities.browser_control.app_mode_enabled` (default: false)
- `capabilities.browser_control.registry_gc_enabled` (default: false)
...
- Contrato da capability e output real ficam consistentes.
