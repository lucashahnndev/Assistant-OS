...
class VisionCapability(CapabilityBase):
    ...
    def _normalize_llm_output(result: Any, default_message: str) -> Dict[str, Any]:
        if isinstance(result, dict):
            ...
            normalized["analysis"] = normalized.get("analysis") or normalized.get("summary") or normalized.get("response") or text_value
            return normalized
    ...
    def _analyze_structured_with_fallback(
        self,
        llm_manager: Any,
        image_path: str,
        prompt: str,
        ...
        **kwargs: Any,
        ...
        try:
            if hasattr(llm_manager, "analyze_image_structured"):
                structured = llm_manager.analyze_image_structured(
            ...
            logger.warning("Structured vision request failed; fallback to legacy parser. error=%s", e)

        raw = llm_manager.analyze_image(image_path, prompt)
        parsed = self._extract_json_object(raw)
        if isinstance(parsed, dict):
            return parsed
        return {"summary": str(raw or ""), "raw": raw}
    ...
    def execute(self, action_id: str, params: Dict[str, Any], context: Dict[str, Any]) -> Any:
        ...
        if action == "analyze":
            ...
            structured = self._analyze_structured_with_fallback(
                llm_manager,
                path,
                prompt,
                contract="vision_analysis_v1",
            )
            normalized = self._normalize_llm_output(
                structured,
                default_message="Image analyzed successfully.",
            )
            ...
            normalized = self._normalize_llm_output(
                structured,
                default_message="Screen analyzed successfully.",
            )
            ...
            locator_data = self._analyze_structured_with_fallback(
                llm_manager,
                screenshot_path,
                prompt,
                contract="vision_locator_v1",
                label=label,
            )
            if not isinstance(locator_data, dict):
                return self._result(
                    ...
                    raw=str(locator_data)[:4000],
                    path=screenshot_path,
                )

            bbox_payload = locator_data.get("bbox") if isinstance(locator_data.get("bbox"), dict) else locator_data
            found = bool(locator_data.get("found", True))
            bbox = self._normalize_bbox(bbox_payload, fallback_label=label)
            bbox = self._normalize_bbox_anchor(bbox_payload, bbox)
            corrected = False
            ...
            if image_size and not self._bbox_inside_image(bbox, image_size[0], image_size[1]):
                ...
                retry_data = self._analyze_structured_with_fallback(
                    llm_manager,
                    screenshot_path,
                    retry_prompt,
                    contract="vision_locator_v1",
                ...
                retry_payload = retry_data.get("bbox") if isinstance(retry_data.get("bbox"), dict) else retry_data
                if isinstance(retry_payload, dict) and bool(retry_data.get("found", True)):
                    retry_bbox = self._normalize_bbox(retry_payload, fallback_label=label)
                    retry_bbox = self._normalize_bbox_anchor(retry_payload, retry_bbox)
                ...
                if not self._bbox_inside_image(bbox, image_size[0], image_size[1]):
                    ...
                    if overlap < 0.25:
                        ...
                        recover_data = self._analyze_structured_with_fallback(
                            llm_manager,
                            screenshot_path,
                            recover_prompt,
                            contract="vision_locator_v1",
                        ...
                        recover_payload = recover_data.get("bbox") if isinstance(recover_data.get("bbox"), dict) else recover_data
                        if isinstance(recover_payload, dict) and bool(recover_data.get("found", True)):
                            recover_bbox = self._normalize_bbox(recover_payload, fallback_label=label)
                            recover_bbox = self._normalize_bbox_anchor(recover_payload, recover_bbox)
                            recover_bbox, recover_scaled, _recover_space = self._maybe_convert_bbox_coordinate_space(
                                parsed_payload=recover_payload,
                                bbox=recover_bbox,
                                img_w=image_size[0],
                                img_h=image_size[1],
                                label=label,
