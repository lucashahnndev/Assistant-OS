...
class PromptComposer:
    ...
    def _build_execution_policy(self) -> str:
        return (
            ...
            "- Infer browser intent when clear; ask at most one concise clarification only if completion criteria are truly ambiguous.\n"
    ...
    def _build_cognitive_blocks(cognitive_frame: Dict[str, Any]) -> Tuple[str, str]:
        ...
        primary_task = cognitive_frame.get("primary_task") or {}
        if primary_task:
            summary = str(primary_task.get("summary") or "").strip()
            focus_lines.append(
                "primary="
                f"[{primary_task.get('task_id', '?')}] "
                f"{primary_task.get('role', 'task')}|{primary_task.get('status', 'unknown')}"
        ...
        secondary_tasks = []
        for task in list(cognitive_frame.get("secondary_tasks") or [])[:4]:
            task_id = str(task.get("task_id") or "?")
            role = str(task.get("role") or "task")
            status = str(task.get("status") or "unknown")
            secondary_tasks.append(f"[{task_id}] {role}|{status}")
        if secondary_tasks:
            background_lines.append("secondary=" + " ; ".join(secondary_tasks))
