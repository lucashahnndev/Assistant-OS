...
class PromptComposer:
    ...
    def compose(
        ...
        *,
        ...
        operating_model_block = self._build_operating_model_block()
        _append("operating_model", operating_model_block)
        ...
        if situational_context:
            _append(
                "situational_context",
                "[SITUATIONAL CONTEXT]\n"
                f"{self._clip_block('situational_context', situational_context)}"
            )
    ...
    def _build_operating_model_block() -> str:
        rules = [
            "You operate inside A.T.L.A.S, not as a generic text-only chatbot.",
            "You are the operator; the runtime is the gatekeeper.",
    ...
    def _build_structured_output_contract(self) -> str:
        return (
            ...
            "- Same action+params fails 3x => clarify."
        )
    ...
    def _legacy_execution_policy() -> str:
        return (
            ...
            "  - If missing information blocks a safe, useful, or correct first step, ask one concise clarification in persona.\n"
    ...
    def _extract_discovery_mode(capabilities_summary: str) -> str:
        lines = str(capabilities_summary or "").splitlines()
        if not lines:
            return "unknown"
